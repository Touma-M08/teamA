<x-app-layout>
  ...
  <div class="max-w-screen-xl px-4 md:px-8 mx-auto">
      ...
      @if( auth() -> user() )
        ...
        <div id="dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
              ...
              <li>
                <a href="{{route('logout')}}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">お店を登録する</a>
              </li>
            </ul>
        </div>
      @else
    ...
    <div class="mb-10 md:mb-16">
      <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6 mt-5">お気に入り一覧</h2>
      ...
      <div class="flex flex-col md:flex-row items-center border rounded-lg overflow-hidden">
        ...
        <div class="flex flex-col gap-2 p-4 lg:p-6">
          ...
          <h2 class="text-gray-800 text-xl font-bold">
            <a href="#" class="hover:text-indigo-500 active:text-indigo-600 transition duration-100">{{$favorite->place->name}}</a>
          </h2>

          <p class="text-gray-500">{{$favorite->place->address}}</p>

          <div>
            <a href="/places/{{$favorite->place->id}}" class="text-indigo-500 hover:text-indigo-600 active:text-indigo-700 font-semibold transition duration-100">お店の詳細を見る</a>
          </div>
        </div>
      </div>
      @endforeach
